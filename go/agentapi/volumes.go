...
func (s *VolumesService) List(ctx context.Context, params ListParams, opts ...RequestOption) (*ListVolumesResponse, error) {
	...
	err := s.http.requestJSON(ctx, "GET", "/v1/volumes"+buildQuery(map[string]any{"limit": params.Limit, "page_token": params.PageToken, "user_id": params.UserID}), nil, &out, opts...)
	return &out, err
}
...
func (s *VolumesService) UpdateWithParams(ctx context.Context, volumeID string, params UpdateVolumeParams, opts ...RequestOption) (*VolumeInfo, error) {
	...
	var out VolumeInfo
	err := s.http.requestJSON(ctx, "PATCH", "/v1/volumes/"+url.PathEscape(volumeID), body, &out, opts...)
	return &out, err
}
