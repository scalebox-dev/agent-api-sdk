...
- Restored `LocalFileStore.list()` to strict complete-or-fail scan semantics while keeping `list_with_warnings()` as the explicit tolerant partial-scan API.
- Made local context packages leave recursive scan depth uncapped unless callers provide `max_depth`.
